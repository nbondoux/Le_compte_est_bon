...
let rec algo l prof best_res  cible=
  let i = ref 0 in
    while !i <  (Array.length l) && (valeur_Noeud(l.(!i)) != cible)
    do
      i := !i + 1
    done;
    if !i!= (Array.length l) then
      let profa = profondeur_arbre l.(!i) in 
	(
	  match best_res with
	      BestArbre (a,old_prof) ->  
		if  (profa < old_prof) || (poids_arbre l.(!i)) < (poids_arbre a) then
		  (
(*		    print_int cible;print_string (" = "^(string_arbre l.(!i))^" ; "^string_of_int(profa));print_endline "";*)
		    BestArbre (l.(!i),profa )
		  )
		else best_res
...
let main()=
  ...
  if(longueur <3) then
    ...
    begin
      if (
	let i=ref 1 in
	  while !i <longueur &&  (String.compare Sys.argv.(!i) "-h")!=0  do
	    i:= !i+1
	  done;
	  !i !=longueur
      )
