...
{
  if((p->arbre_pool[pr-1])->nb_refs>1) //si l'arbre est utilisé ailleurs
    {
...
void algo2 (arbre ** l,unsigned int taille_l, unsigned int prof, solution * best_res, int cible, malloc_arbre_pool* p)
{
  ...
  // optimisation du pool: on s'assure que le noeud actuel du pool ne reference aucun autre noeud, et ainsi pourra être réutilisé plus tard
