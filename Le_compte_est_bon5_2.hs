...
getSubCombinationsFixedLSize iL iL1Size iLSize =
  if iL1Size == 0 then
    ...
    case iL of 
      h:t ->  
        let listsWithH = map (\l -> h:l) (getSubCombinationsFixedLSize t (iL1Size - 1) (iLSize -1 )) in
        if iL1Size < iLSize then
          listsWithH++(getSubCombinationsFixedLSize t (iL1Size) (iLSize - 1))
        else
          listsWithH
      [] -> []
...
getSubCombinations iL iLSize iMinSize iMaxSize =
  let getSubCombinations_rec n =
        if n >= iMaxSize then
          ...
          (getSubCombinationsFixedLSize iL n iLSize)++(getSubCombinations_rec (n+1))
  in
...
getAllSubCombinationCouples iL iLSize =
  let f i =
        if i *2 <= iLSize then
          (getSubCombinationCouplesFixedL1Size_bis 
           iL i iLSize)++(f (i+1))
        else
          [] in
  f 0;;
